...
delta_P_dis = sdpvar(NOFEV, NOFTCAP, 'full'); % 偏差功率(kW)
delta_P_ch = sdpvar(NOFEV, NOFTCAP, 'full'); % 偏差功率(kW)
...
% 老化费用不影响结果，只是为了避免同时充放电。
Cost_s = sum(param.Pr_deg' * P_dis_s) + M * sum(sum(delta_P_dis + delta_P_ch));
...
Constraints = [Constraints, 1e3 * repmat(Bid_P_cur', 1, NOFTCAP) + ...
    1e3 * repmat(Bid_R_cur', 1, NOFTCAP) .* repmat(delta, NOFEV, 1) - ...
    (delta_P_dis - delta_P_ch + P_dis_s - P_ch_s) == 0];
...
x = param.E_leave - repmat(param.eta, 1, NOFTCAP) .* REST_TIME * param.P_max;
